...
const ASSETS=[
  ...
  {n:'X / Twitter card',d:'1200×675',t:'social',bg:'#1A1A2E',tc:'#F4A26B',s:'X Card'},
  ...
  {n:'Email header',d:'600×200',t:'email',bg:'#1A1A2E',tc:'#F4A26B',s:'Email Header'},
  {n:'Email footer',d:'600×100',t:'email',bg:'#F0EDE8',tc:'#1A1A2E',s:'Email Footer'},
  ...
  {n:'Skyscraper',d:'160×600',t:'display',bg:'#1A1A2E',tc:'#F4A26B',s:'Sky'},
];
...
function goStrategy(){showPage('strategy')}
function goContent(){showPage('content')}
function goAssets(){showPage('assets')}
function goHome(){showPage('home')}

/* ── SIDEBAR ── */
function setSidebarActive(id){
  document.querySelectorAll('.sidebar-link').forEach(l=>l.classList.remove('active'));
  const el=document.querySelector(`.sidebar-link[data-sid="${id}"]`);
  if(el)el.classList.add('active');
}
function sideNav(id){
  setSidebarActive(id);
  if(id==='setup')showPage('setup');
  else if(id==='strategy')showPage('strategy');
  else if(id==='content')showPage('content');
  else if(id==='assets')showPage('assets');
}
...
function enGen(){
  const b=gi('gen-btn');
  if(b){b.disabled=false;b.textContent=isGen?'Regenerate strategy and plan':'Start engine';}
}
['i-brand','i-ind','i-desc','i-obj','i-aud','i-tone','i-bb','i-tagline','i-prompt'].forEach(id=>{
  const el=gi(id);if(el)el.addEventListener('input',enGen);
});
...
async function generate(){
  ...
  const b=gi('gen-btn');b.disabled=true;b.textContent='Generating...';
  isGen=true;
  bAssets();
  showPage('strategy');setSidebarActive('strategy');
  await bStrat(inp);
  bCal();
}
...
async function bStrat(inp){
  gi('strat-inner').innerHTML=`<div class="loading"><div class="spin"></div><div class="load-txt">Building your strategy...</div><div class="load-step" id="ls">Analysing brand inputs</div></div>`;
  ...
  const tN=inp.tagline?`\nUse this VERBATIM as "headline": "${inp.tagline}"`:'' ;
  const pN=inp.prompt?`\nClient direction: ${inp.prompt}`:'';
  const prompt=`Senior marketing strategist. No generic phrases allowed.\nBrand:${inp.brand}, Industry:${inp.ind}, Description:${inp.desc}, Objective:${inp.obj}, Audience:${inp.aud}, Budget:${inp.bud}/mo, Voice:${inp.tone}, Channels:${inp.ch.join(',')}, Guidelines:${inp.bb}${tN}${pN}\n\nRules:\n1. headline = tagline verbatim if given, else sharp brand-specific max 12 words — NO generic phrases\n2. summary = concrete rationale naming audience tension + channel logic\n3. each phase desc = 2+ specific named tactics\n4. messages = differentiated claims ONLY ${inp.brand} can make\n5. kpis = realistic projected ranges for ${inp.bud}/mo\n\nReturn ONLY JSON no markdown:\n{"headline":"...","summary":"...","phases":[{"week":"Weeks 1-2","name":"...","desc":"..."},{"week":"Weeks 3-4","name":"...","desc":"..."},{"week":"Weeks 5-6","name":"...","desc":"..."},{"week":"Weeks 7-8","name":"...","desc":"..."}],"alloc":[{"lbl":"Paid social","pct":35},{"lbl":"Content creation","pct":25},{"lbl":"Email / CRM","pct":15},{"lbl":"Influencer / seeding","pct":15},{"lbl":"Contingency","pct":10}],"kpis":[{"lbl":"Projected monthly reach","val":"40-60K"},{"lbl":"Projected engagement","val":"3.5-5.5%"},{"lbl":"Projected email open","val":"24-32%"}],"messages":["...","...","..."]}`;
  try{
    const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1600,messages:[{role:'user',content:prompt}]})});
    const data=await r.json();clearInterval(iv);
    if(data.error)throw new Error(data.error.message);
    const txt=data.content.map(c=>c.text||'').join('');
    rStrat(JSON.parse(txt.replace(/```json|```/g,'').trim()),inp);
  }catch(e){clearInterval(iv);rStrat(fb(inp),inp);}
  const b=gi('gen-btn');if(b){b.disabled=false;b.textContent='Regenerate strategy and plan';}
}
...
function fb(inp){
  ...
  return{
    ...
    summary:`${br} enters a crowded ${inp.ind} market with one clear advantage: ${inp.desc?inp.desc.split('.')[0]:inp.obj}. This 8-week strategy prioritises channel-specific storytelling over broadcast volume — building trust with ${a} before asking for conversion. Every budget dollar is tied to a measurable outcome.`,
    phases:[
      {week:'Weeks 1-2',name:'Foundation and authority',desc:`Publish a 1,200-word founder essay on LinkedIn establishing category expertise. Launch a 3-part Instagram carousel on brand origin story. Set up pixel tracking, email capture with 10% discount trigger, and 3-email automated welcome sequence.`},
      {week:'Weeks 3-4',name:'Proof and amplification',desc:`Activate paid social on the top-performing organic post. Publish 2 customer testimonials on Instagram Stories and 1 case study on LinkedIn. A/B test two email subject lines. Seed 3-5 micro-influencers in the ${inp.ind} space.`},
      {week:'Weeks 5-6',name:'Conversion push',desc:`Scale paid social on the winning creative variant. Deploy retargeting to all site visitors and email openers. Run a 3-email limited-time offer sequence. Publish an SEO-targeted blog post for the primary ${inp.ind} search query.`},
      {week:'Weeks 7-8',name:'Retention and optimisation',desc:`Launch referral mechanic for new customers via post-purchase email. Publish campaign results on LinkedIn for brand credibility. Test 2 new ad formats. Document CPL and ROAS to inform the next campaign cycle.`},
    ],
    ...
    messages:[
      `${br} is the only ${inp.ind} brand that ${inp.desc?inp.desc.split('.')[0].toLowerCase().replace(/^we /,''):'starts from the customer, not the category'}`,
      `Designed for ${a} — not adapted from a mass-market template`,
      `Every claim is verifiable: we publish the evidence, not just the headline`,
    ],
  };
}
...
function rStrat(s,inp){
  ...
  <div class="kpi-row">
    ${s.kpis.map((k,i)=>`<div class="kpi kpi-${i}"><div class="kpi-lbl">${k.lbl}</div><div class="kpi-val">${k.val}</div></div>`).join('')}
  </div>
  <div class="s-hd"><div class="s-title">8-week campaign phases</div><div class="s-rule"></div><div class="s-tag">56 days</div></div>
  <div class="phases">
    ${s.phases.map((p,i)=>`<div class="phase"><div class="ph-week">${p.week}</div><div class="ph-line"><div class="ph-node"></div>${i<s.phases.length-1?'<div class="ph-track"></div>':''}</div><div class="ph-body"><div class="ph-name">${p.name}</div><div class="ph-desc">${p.desc}</div></div></div>`).join('')}
  </div>
...
/* ── CALENDAR ── */
function getChipOn(){
  return{ig:gi('ch-ig').classList.contains('on-ig'),li:gi('ch-li').classList.contains('on-li'),em:gi('ch-em').classList.contains('on-em'),bl:gi('ch-bl').classList.contains('on-bl')};
}
function toggleChip(id,cls){const el=gi(id);el.classList.toggle(cls);bCal();}
...
async function gPost(title,type){
  ...
  const p=`Write a ${tl} for ${inp.brand||lb} (${inp.ind}). Concept: "${title}". Voice: ${inp.tone}. Audience: ${inp.aud}. Instagram: add 3-5 hashtags. Email: subject line first. Return ONLY the copy.`;
  try{
...
/* ── ASSETS ── */
function bAssets(){
  const brand=lb!=='Your Brand'?lb:(gi('i-brand').value.trim()||'Your Brand');
  ...
  g.innerHTML=ASSETS.filter(a=>atf==='all'||a.t===atf).map(a=>`
    <div class="acard" data-type="${a.t}" data-name="${a.n.toLowerCase()}">
      ...
      <div class="ainfo">
        ...
        <div class="abtn-row">
          <button class="abtn" onclick="alert('Resize ${a.n} (${a.d}):\\n\\nIG: 1080x1080/1350 | Story: 1080x1920\\nLI: 1200x627 | X: 1200x675 | FB: 1200x630')">Resize</button>
          <button class="abtn" onclick="alert('Spec: ${a.n}\\nFormat: PNG-24 (web) / PDF (print)\\nColour: sRGB / CMYK\\nRes: 72dpi / 300dpi\\nSafe zone: 5% inset')">Spec</button>
        </div>
      </div>
    </div>`).join('');
}
